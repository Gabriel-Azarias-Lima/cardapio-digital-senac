...
let cart = [] // carrinho
const CART_STORAGE_KEY = 'senac_pizzaria_cart'

const carregarCarrinhoStorage = () => {
    try {
...
document.querySelectorAll('input[name="tipoPedido"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
        ...
        enderecoArea.style.display = isEntrega ? 'block' : 'none'

        // Marcar campos obrigatórios apenas para entrega
        const camposObrigatorios = ['#enderecoRua', '#enderecoNumero', '#enderecoBairro', '#enderecoCidade']
        camposObrigatorios.forEach(selector => {
            const campo = seleciona(selector)
            if (campo) campo.required = isEntrega
        })
    })
})
...
// Confirmar pedido
seleciona('#confirmarPedido').addEventListener('click', async () => {
    ...
    if(tipoPedido === 'entrega') {
        const rua = seleciona('#enderecoRua').value.trim()
        const numero = seleciona('#enderecoNumero').value.trim()
        const bairro = seleciona('#enderecoBairro').value.trim()
        const cidade = seleciona('#enderecoCidade').value.trim()

        if(!rua || !numero || !bairro || !cidade) {
            alert('Por favor, preencha rua, número, bairro e cidade para entrega!')
            return
        }
    }
    ...
    const enderecoPayload = tipoPedido === 'entrega' ? {
        rua: seleciona('#enderecoRua').value.trim(),
        numero: seleciona('#enderecoNumero').value.trim(),
        bairro: seleciona('#enderecoBairro').value.trim(),
        cidade: seleciona('#enderecoCidade').value.trim(),
        referencia: seleciona('#enderecoReferencia').value.trim(),
        observacoes: seleciona('#enderecoObservacoes').value.trim()
    } : null
    ...
    try {
        ...
        if (!response.ok || !result.success) {
            const msg = result && result.message ? result.message : 'Não foi possível finalizar o pedido. Tente novamente.'
            alert(msg)
            return
        }
        ...
        // Limpar carrinho/localStorage e formulário
        cart = []
        atualizarCarrinho()
        ...
        alert('Ocorreu um erro ao finalizar o pedido. Verifique sua conexão e tente novamente.')
    }
})
